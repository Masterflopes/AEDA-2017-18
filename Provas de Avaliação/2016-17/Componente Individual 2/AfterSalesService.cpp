...
bool compareArticles(Article * a1, Article *a2){
	if(a1->getDeliverHome() == true && a2->getDeliverHome() == false)
		return false;
	if(a1->getDeliverHome() == false && a2->getDeliverHome() == true)
		return true;
	if(a1->getDeliverHome() == false && a2->getDeliverHome() == false){
...
void AfterSalesService::enqueueArticles() {
	...
	for(size_t i = 0; i < presentsTable.size(); i++){
		if(toWrap.size() < this->toWrapQueueSize){
...
Article* AfterSalesService::wrapNext() {
	...
	Article * retorno = toWrap.front();

	if(toWrap.empty())
		return NULL;

	if(toWrap.front()->getDeliverHome() == true){
